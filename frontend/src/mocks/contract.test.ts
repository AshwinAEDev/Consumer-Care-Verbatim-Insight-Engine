...
import { getInsightById, getInsights, getVerbatimsByIds } from "../lib/api-client";
import { complaintLabel, formatPercent, leadTimeLabel } from "../lib/labels";
import { weeklyTrend } from "../lib/trend";
import { ISSUE_TYPES, PACK_TYPES } from "../lib/types.generated";
...
describe("mock contracts", () => {
  it("keeps relational ids and the 50-complaint set intact", () => {
    ...
    for (const pack of packs) {
      ...
      expect(PACK_TYPES.includes(pack.type)).toBe(true);
      expect(pack.launchDate).toBeInstanceOf(Date);
    }
    ...
    for (const verbatim of verbatims) {
      ...
      expect(verbatim.issueType !== undefined && ISSUE_TYPES.includes(verbatim.issueType)).toBe(true);
      expect(verbatim.timestamp).toBeInstanceOf(Date);
